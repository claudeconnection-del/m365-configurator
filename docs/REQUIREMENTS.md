...
- [x] **FR-1 — Module lifecycle.** Detect, install (CurrentUser scope), import, and
  report versions of required PowerShell modules. Idempotent; safe to re-run.
- [x] **FR-2 — Connect/disconnect at runtime.** Establish and tear down M365
  connections (Graph, Exchange Online) on demand within a running session.
- [x] **FR-3 — Full credential cleanup.** On disconnect/session end, purge tokens
...
- [x] **FR-5 — Save profiles.** Persist a configuration as a named, versioned,
  human-readable profile containing **configuration only — never credentials**.
- [x] **FR-6 — Load profiles.** Select a saved profile from a list, or import an
  exported profile file (for collaboration).
- [x] **FR-7 — Name remapping.** When applying a profile, allow updating the names
  of name-scoped configurations (names are unique per configuration/tenant).
- [x] **FR-8 — Dry run.** Preview the exact set of changes a profile would make
  against a connected tenant, with a clear pass/needs-attention signal, before
  applying anything.
- [x] **FR-9 — Apply.** Apply a profile only after an explicit go-ahead; report
  per-item success/failure.
- [x] **FR-10 — Drift detection.** Scan a connected tenant against a saved profile
  and report differences.
- [x] **FR-11 — Deterministic remediation.** For detected drift, offer remediation
  that is deterministic and previewable (dry-run applies here too).
- [x] **FR-12 — Log access.** Provide easy, in-tool access to the verbose logs.

## Non-functional requirements (NFR)

- [x] **NFR-1 — No credential persistence.** Credentials/tokens are never written
  to disk, logs, profiles, or telemetry. Verified by test + review.
- [x] **NFR-2 — No unexpected egress.** The app contacts only endpoints it is
...
- [x] **NFR-7 — Syntax-stability.** The tool remains correct as long as the
  ...
  (ADR-0015).
- [x] **NFR-8 — Reproducible dev/build.** A fresh clone reaches a working dev
  environment via the documented bootstrap with no hidden steps.
- [x] **NFR-9 — Readability & visual inspection.** All human-facing output — code,
...
(no-egress, minimal-deps; for NFR-7 the **module-pin half** stays anchored by
`Get-M365RequiredModule` + `scripts/install-modules.ps1` — ADR-0015 records the
**runtime** half). An ADR is written only if one of these becomes a contested,
hard-to-reverse decision — as runtime-pinning did in ADR-0015.
