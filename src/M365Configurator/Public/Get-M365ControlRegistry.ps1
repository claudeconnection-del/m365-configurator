...
function Get-M365ControlRegistry {
    ...
    .DESCRIPTION
        ...
        auto-forward-block, MDO-10 external-sender-tag, AUD-1
        unified-audit-log, and AUD-2 mailbox-audit controls; further Graph
        (MCA-4) and Exchange Online (MCA-5) controls register here as they
        land, each an additive one-line entry (ADR-0013).
